...
def azimuth_from_image(img):
    ...
    if 'two-digit' in preds:
        preds = azimut_digit_detector_clf.predict([preprocess(digit) for digit in get_azimut_digits(get_azimut_image(img),two_digit=True)])
    ...
    if any([v not in '0123456789' for v in pred]):
        return None
